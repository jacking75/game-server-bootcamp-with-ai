...
> 버전: 0.4 — 2026-09-05 리뷰의 명세·일정·템플릿·영문 정책 반영
...
| 문서 | 내용 |
|---|---|
| `README.md` / `README_kr.md` (이 문서) | 과정 목표, 트랙 구조, AI 학습 원칙, 로드맵, 검증 체계, 환경 |
...
기본 진입점 `README.md`와 번호 문서는 영어이며, 같은 이름의 `_kr.md`가 한국어판이다. 각 Phase 문서는 **사용법 → 개요·자가 진단 → 일 단위 계획 → 실습 카탈로그 → 학습 항목 → 교재 → AI 협업 → 과제 → 완료 판정·질문 은행 → 막힘 포인트 → 다음 Phase 준비** 순서다.
...
| 2 | 4~7 | TCP/IP, 패킷 프레이밍, 직렬화, 세션 관리 | Socket/SocketAsyncEventArgs, Pipelines, MemoryPack, SuperSocketLite 비교 | Winsock, select → Overlapped → IOCP, 세션·버퍼 관리 | 다중 접속 채팅 서버 + 패킷 라이브러리 + 테스트 클라이언트 |
...
|---|---|---|---|
| 1 | OpenCode 가이드, C# 디자인 패턴 3장 | Async/Await 1~4·6·7장, async 라이브러리 1~6장 | Modern C++ 2~7·10·12·13장, Windows 멀티스레딩 2~5·8장, Win32 1·2·4~6장, 메모리 모델 1~2부 |
| 2 | 네트워크 입문 2권, 로드맵 1·2·7장 | C# Socket 1~7·9·10장, SuperSocketLite 1~6장, async 라이브러리 7장 | TCP/IP Windows 소켓 1~6·8~11·13장, 선택 Boost.Asio 1·2·4~7장 |
| 3 | 로드맵 3~5장, 디자인 패턴 7~12·16장, 매칭 1·3장, 클라이언트 네트워크 4·5장 | C# Socket 11장, SuperSocketLite 7장, async 라이브러리 16~18장, Async/Await 10장 | TCP/IP 소켓 11·14장, Windows 멀티스레딩 6·11·12장, 선택 Boost.Asio 11~13·17장 |
| 4 | MySQL Day 1·2·5, Redis 1·2장, API 실습 부록 A~C·F·G, 가챠 9~12·16장 | API 실습 1~14·16~18·20·22장, WebAPI 5~8·13·16장, MySQL 3~7, Redis 3~5·7, 2D MMORPG 2주차 | (a) 공식 문서, (b) C# 혼합 경로 동일 |
| 5 | Prometheus 전체, fluentd 1·2장 통독(나머지 참조), 로드맵 5·6장, API 실습 23·24장 | Async/Await 5장·부록 B, async 라이브러리 14·19·20장 | Windows 멀티스레딩 10장, Win32 7장, TLS 4~6장, 메모리 모델 3·4부, TCP/IP 12~14장 |
| 6 | 로드맵 8장, 매칭 4~7장, 2D MMORPG 참조 구조 | 2D MMORPG 코드, ECS 선택 | 선택 Boost.Asio 17·20·21장 |
...
- 매주 금요일 오후 4시간 주간 점검
  1. AI 없이 재구현: Phase 난이도에 따라 핵심 컴포넌트 하나를 60~120분 내 재구현
  2. 설명 시험: AI 면접관에게 이번 주 주제 3개를 60분 설명·채점
...
| 설명 시험 | AI 면접관 60분 심층 질문 | 5점 척도 평균 4.0 이상 |
...
|---|---|---|
| 2 | 채팅 서버 | TCP 9000 |
| 3 | 게임 서버·부하 클라이언트 | TCP 5000/5001, 도구 5050 |
| 4 | API 서버 | HTTP 8080(캡스톤과 충돌 시 8081) |
| 5 | 메트릭 | HTTP 9101 |
| 6 | API·매칭·게임 2대 | 8080·8090·9001·9002 |

포트와 `serverId`는 설정/CLI 인자로 바꿀 수 있어야 한다. 부하 클라이언트의 동적 포트는 `netsh int ipv4 show dynamicport tcp`로 별도 확인한다.

## 8. ADR 번호표

| 번호 | 주제 | Phase/결정 시점 |
|---|---|---|
| 0001 | 직렬화 | Phase 2, 20일차 |
| 0002 | 룸 스레드 모델 | Phase 3, 37일차 |
| 0003 | 룸 간 메시지 | Phase 3, 39일차 |
| 0004 | 재접속 인증 | Phase 3, 46일차 |
| 0005 | DB 추상화 | Phase 4, 57일차 |
| 0006 | 비밀번호 해시 | Phase 4, 59일차 |
| 0007 | 세션 토큰 회전 | Phase 4, 61일차 |
| 0008 | 재화·재고 단일 진실 | Phase 4, 66일차 |
| 0009 | 결과 큐 내구성 | Phase 5, 91일차 |
| 0010~0012 | 서버 간 통신·매칭·티켓 | Phase 6, 103~107일차 |
| 0013 | 장애 복구 방식 | Phase 6, 112일차 |
| 0014 | 결과 저장 내구성 | Phase 6, 116일차 |
| 0015 | 캡스톤 범위 동결 | Phase 6, 120일차 |
